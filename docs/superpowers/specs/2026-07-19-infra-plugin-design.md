...
키·인증서를 표 또는 목록으로 관리. 항목: 이름, 종류(ssh | cloud | api-token | tls-cert), fingerprint, **위치 참조**, 소유자, 생성일, 만료/로테이션 예정일. TLS 인증서 만료도 여기서 추적한다.

위치 참조는 외부(`~/.ssh/deploy-key`, `op://vault/item`)일 수도, 하네스 내부(`secrets/vm-token.age`)일 수도 있다 — 내부 보관 가능 여부는 원칙 2의 공유 모드 규칙을 따른다. 값은 이 파일에 절대 적지 않는다.
...
유효성 규칙: `secrets_mode: plaintext`는 `sharing: local`에서만 허용된다. 위반 조합은 audit가 실패 처리한다. `policies.mutating`에 없는 env는 `confirm`으로 취급한다(D4).
